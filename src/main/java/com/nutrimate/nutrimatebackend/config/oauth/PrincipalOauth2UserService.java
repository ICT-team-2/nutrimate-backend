...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
  ...
  public PrincipalOauth2UserService(MemberMapper memberMapper, PasswordEncoder passwordEncoder) {
  ...
  public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
    ...
    String userUid = provider + "_" + providerId;// google_
    String userPwd = passwordEncoder.encode("겟인데어");
    String email = oAuth2UserInfo.getEmail();
    String role = "ROLE_USER";


    MemberDto userEntity = memberMapper.findByUser(userUid);

    if (userEntity == null) {
      System.out.println("OAuth 로그인이 최초");
      userEntity = MemberDto.builder().userId(userUid).userPwd(userPwd).userEmail(email)
          .userRole(role).provider(provider).providerId(providerId).build();

      memberMapper.insertMember(userEntity);
    } else {
