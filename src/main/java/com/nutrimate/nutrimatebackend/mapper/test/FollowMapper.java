...
public interface FollowMapper {
  ...
  // @Select("SELECT user_id, user_profile, user_nick, user_intro FROM (SELECT * FROM member ORDER
  // BY DBMS_RANDOM.VALUE) WHERE ROWNUM <= 5")
  List<FollowDto> getRecommendedFollowers();

  // 내가 상대를 팔로우 하는 쿼리문 (내가 팔로워가 된다)
  // @Insert("INSERT INTO follow (follow_id, follower_id, created_date, followee_id) VALUES
  // (SEQ_FOLLOW.NEXTVAL, #{followerId}, SYSDATE, #{followeeId})")
